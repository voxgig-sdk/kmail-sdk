...
[MCP](https://modelcontextprotocol.io) server exposing the Kmail SDK as
two agent tools — `kmail_list` and `kmail_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kmail-mcp)
make build

# 2. Provide credentials via the environment
export KMAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kmail \
  -- /absolute/path/to/kmail-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kmail-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kmail_list: first page of records
{ "entity": "get_email" }
{ "entity": "get_email", "query": { } }

// kmail_load: one record by id
{ "entity": "get_email", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kmail-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kmail \
     -- "$PWD"/dist/*/kmail-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kmail_list` and `kmail_load` tools now appear
   in new sessions. Ask the agent to *"list get_email using kmail"*
   and it calls `kmail_list` with `{"entity":"get_email"}`.
...
```sh
export KMAIL_APIKEY=sk_live_xxx            # API key
export KMAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `kmail_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kmail_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KMAIL_APIKEY` | API key sent with every request. |
| `KMAIL_BASE` | Optional override of the API base URL. |
...
```sh
./kmail-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kmail_load","arguments":{"entity":"get_email","query":{"id":1}}}}'
```
