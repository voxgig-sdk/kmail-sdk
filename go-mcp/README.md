...
MCP server exposing the Kmail SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `kmail_list` | `entity`, optional `query` map | First page of records as JSON |
| `kmail_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `kmail_list` and
`kmail_load` tools become available in new sessions.
