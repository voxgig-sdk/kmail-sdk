...
from kmail_sdk.kmail_types import (
    GetEmail,
    GetEmailListMatch,
)
...
class GetEmailEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetEmailEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[GetEmail]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.GetEmail().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
