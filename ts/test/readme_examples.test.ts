...
//     (\`new KmailSDK(...)\` / \`KmailSDK.test(...)\` become
//     \`KmailSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_email":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KmailSDK(...)\` and any
// \`KmailSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KmailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KmailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
