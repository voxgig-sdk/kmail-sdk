...
class KmailSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KmailHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KmailHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, KmailFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KmailUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KmailHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KmailHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KmailHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KmailError($op . "_allow",
                "KmailSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KmailHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KmailError("graphql_error",
                "KmailSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetEmail($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_email === null) {
                $this->_get_email = new GetEmailEntity($this, null);
            }
            return $this->_get_email;
        }
        return new GetEmailEntity($this, $data);
    }
