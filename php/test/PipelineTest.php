...
{
    private static function utility(): KmailUtility
    {
        return new KmailUtility();
    }
    ...
    private static function ctx(array $over = []): KmailContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KmailContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KmailOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KmailError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KmailOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KmailMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KmailOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KmailMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KmailOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KmailMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KmailSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KmailMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KmailResponse([]),
            'result' => new KmailResult([]),
        ]);
        [, $err] = KmailMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KmailSpec(['step' => 's']),
            'response' => new KmailResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KmailResult([]),
        ]);
        [, $err] = KmailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KmailSpec(['step' => 's']),
            'response' => new KmailResponse(self::resp(200, ['v' => 1])),
            'result' => new KmailResult([]),
        ]);
        [, $err] = KmailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KmailSpec(['step' => 's']),
            'response' => new KmailResponse(self::resp(200, ['v' => 2])),
            'result' => new KmailResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KmailMakeResponse::call($ctx);
    ...
    {
        $preset = new KmailResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KmailSpec([]),
            'response' => new KmailResponse([]),
            'result' => new KmailResult([]),
        ...
        [$got, $err] = KmailMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KmailResult([])]);
        [, $err] = KmailMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KmailSpec([]), 'result' => null]);
        [, $err] = KmailMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KmailSpec(['step' => 's']),
            'result' => new KmailResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KmailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KmailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KmailSpec(['step' => 's']),
            'result' => new KmailResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KmailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KmailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KmailResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KmailSpec([]),
            'result' => new KmailResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KmailMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new KmailError('boom', 'boom');
        $utility->fetcher = function (KmailContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KmailSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KmailMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KmailContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KmailResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KmailContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KmailSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KmailResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KmailSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KmailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KmailMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KmailMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KmailResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KmailDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KmailResult(['ok' => false])]);
        $this->expectException(KmailError::class);
        KmailDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KmailResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KmailMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KmailBaseFeature();
        $b = new KmailBaseFeature();
        KmailFeatureAdd::call($ctx, $a);
        KmailFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KmailBaseFeature
    {
        $f = new KmailBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): KmailContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KmailSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new KmailResponse(['headers' => null]),
            'result' => new KmailResult([]),
        ]);
        KmailResultHeaders::call($ctx);
