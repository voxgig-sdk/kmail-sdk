...
// Kmail SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        KmailSDK $client,
        KmailUtility $utility
    ): KmailContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new KmailContext($ctxmap, null);
    ...
    private static function fixctx(KmailContext $ctx, KmailSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?KmailError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KmailError($code, $msg);
    }
    ...
    {
        $hook_client = KmailSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends KmailBaseFeature {
            public $hook_fn = null;
            public function TestHook(KmailContext $ctx): void
            {
    ...
    {
        $init_client = KmailSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends KmailBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new KmailSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = KmailSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = KmailSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof KmailError) ? $result->err->msg : (string)$result->err;
