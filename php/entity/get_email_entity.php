...
class GetEmailEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetEmailEntity($this->_client, $opts);
    }

    /**
     * @param GetEmail|array $args GetEmail data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KmailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetEmail|array The current GetEmail data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetEmail fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KmailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetEmail fields).
     */
    ...
    /**
     * List GetEmail items matching the given filter.
     *
     * @param GetEmailListMatch|array|null $reqmatch Match filter (any subset
     *   of GetEmail fields) as an assoc-array; GetEmailListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetEmail[]|array A list of GetEmail items as assoc-arrays at
     *   the SDK boundary; throws KmailError on failure (item-5 convention).
     */
