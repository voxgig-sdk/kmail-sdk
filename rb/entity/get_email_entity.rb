...
class GetEmailEntity
  ...
  def make
    opts = @_entopts.dup
    GetEmailEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = KmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetEmail, Hash] the current GetEmail data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = KmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetEmail fields)
  def match_get
  ...
  #
  # @param reqmatch [GetEmailListMatch, Hash, nil] match filter (any subset of
  #   GetEmail fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetEmail>, Array] the matching GetEmail items; raises KmailError on failure
