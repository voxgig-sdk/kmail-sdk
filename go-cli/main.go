// Command Kmail-cli is a CLI + REPL for the Kmail SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KMAIL_APIKEY (api key), KMAIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KMAIL_APIKEY carries the API key and
	// KMAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KMAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KMAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKmailSDK(opts)
