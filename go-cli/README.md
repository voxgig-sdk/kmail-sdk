...
boru-driven command-line client **and** interactive REPL for the Kmail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kmail-cli)
make build
...
./kmail-cli list get_email

# 5. Override the API base URL for a single call
KMAIL_BASE=https://api.example.com ./kmail-cli list get_email

# 6. No arguments -> interactive REPL
./kmail-cli
kmail> list get_email
kmail> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kmail-cli
   ```
...
Run with no arguments to open a REPL (prompt `kmail>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_email`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kmail-cli`. |
...
(`./kmail-cli <expr>`) and interactively (the REPL), and expressions compose the
