...
The Kmail API provides a web interface for kmail.pw, a professional temporary email service.

Learn more about Kmail API at [kmail.pw](https://kmail.pw).

This is an unofficial SDK for the Kmail public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — GetEmail — that you
...
const items = await client.GetEmail().list()
```
...
```go
client := sdk.Test()
result, err := client.GetEmail(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **GetEmail** | The GetEmail entity (list). | `/get_email` |
