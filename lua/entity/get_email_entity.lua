...
local GetEmailEntity = {}
GetEmailEntity.__index = GetEmailEntity


function GetEmailEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetEmailEntity)
  self._name = "get_email"
  self._client = client
...
function GetEmailEntity:get_name()
  return self._name
end


function GetEmailEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetEmailEntity.new(self._client, opts)
end
...
function GetEmailEntity:mark_deleted()
  self._deleted = true
end


function GetEmailEntity:deleted()
  return true == self._deleted
end


function GetEmailEntity:data_set(args)
  if args ~= nil then
...
function GetEmailEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetEmailEntity:match_set(args)
  if args ~= nil then
...
function GetEmailEntity:match_get()
...
function GetEmailEntity:stream(action, args, callopts)
...
---@param reqmatch GetEmailListMatch
---@param ctrl? table
---@return GetEmail[]
---@return string? err
function GetEmailEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function GetEmailEntity:_run_op(ctx, post_done)
  local utility = self._utility
