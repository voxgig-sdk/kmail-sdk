...
class FtCtrl extends KmailControl
{
...
class FtClient
{
    ...
    private KmailUtility $utility;

    public function __construct(KmailUtility $utility, array $options)
    {
    ...
    public function get_utility(): KmailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KmailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KmailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KmailConfig::make_feature($name);
    ...
    private static function buildUrl(KmailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KmailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KmailResponse($fetched);
        }
    ...
    private function fail(KmailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KmailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
